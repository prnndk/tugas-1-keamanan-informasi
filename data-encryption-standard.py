...
# Remove PKCS5 padding from the plaintext
def unpad(text):
    padding_len = ord(text[-1])
    return text[:-padding_len]

def split_blocks(text, block_size=8):
...
def encrypt(pt, rkb, rk):
    ...
    for block in blocks:
        block_hex = str2hex(block)
        cipher_text = encrypt_block(block_hex, rkb, rk)
        encrypted_blocks.append(bin2hex(cipher_text))
